...
def register_crypt_handler_path(name, path):
    ...
    for a class named ``myhash`` within the ``mybfastapi.apphelpers`` module::

        >>> from passlib.registry import registry_crypt_handler_path
        >>> registry_crypt_handler_path("myhash", "mybfastapi.apphelpers")

    ...while this form would cause ``get_handler("myhash")`` to look
    for a class name ``MyHash`` within the ``mybfastapi.apphelpers`` module::

        >>> from passlib.registry import registry_crypt_handler_path
        >>> registry_crypt_handler_path("myhash", "mybfastapi.apphelpers:MyHash")
    """
    # validate name
